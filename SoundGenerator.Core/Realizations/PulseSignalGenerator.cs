...
public class PulseSignalGenerator : ISignalGenerator
{
    public IEnumerable<double> GenerateAmplitudeModulatedSignal(IEnumerable<double> messageSignal, GeneratorData carrierData)
    {
        ...
        var dutyCycle = carrierData.DutyCycle;
        var messageSignalList = messageSignal.ToList();
        List<double> modulatedSignal = new();
        for(int i = 0; i < sampleRate * time; i++)
        {
            var modulatedSignalValue = GenerateSignalValue(carrierAmplitude * (1 + messageSignalList[i]), carrierFrequency, i, sampleRate, carrierInitialPhase, dutyCycle);
    ...
    public IEnumerable<double> GenerateFrequencyModulatedSignal(IEnumerable<double> messageSignal, GeneratorData carrierData)
    {
        ...
        for (int i = 0; i < sampleRate * time; i++)
        {
            ...
            var modulatedSignalValue = GenerateSignalValue(carrierAmplitude, carrierFrequency * sum, 1, sampleRate, carrierInitialPhase, dutyCycle);
            modulatedSignal.Add(modulatedSignalValue);
        }
        return modulatedSignal;
    }
    ...
    public IEnumerable<double> GenerateSignal(GeneratorData generatorData)
    {
        ...
        var dutyCycle = generatorData.DutyCycle;
        var time = generatorData.Time;
        for (int i = 0; i < sampleRate * time; i++)
        {
            var signalValue = GenerateSignalValue(amplitude, frequency, i, sampleRate, initialPhase, dutyCycle);
